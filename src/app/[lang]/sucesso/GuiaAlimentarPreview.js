...
export default function GuiaAlimentarPreview({
  ...
  const Page = ({ num, children, isSmall = false, withPattern = false, padding = '15px' }) => (
    <div style={{ 
      ...
      <div style={{ position: 'absolute', top: '5px', right: '5px', fontSize: '6px', color: '#ccc', fontWeight: 700, zIndex: 10 }}>
        {gTranslation?.pag_label || 'PÁG'} {num} {num === 1 ? (gTranslation?.capa_label || '(CAPA)') : ''}
      </div>
    ...
    <div id="folder-trifold-preview" style={{ display: 'flex', flexDirection: 'column', gap: '30px', width: '100%', alignItems: 'center', paddingBottom: '40px' }}>
      ...
      {setFolderRoof && (
        <button onClick={() => setFolderRoof(v => !v)} style={{ fontSize: '0.7rem', padding: '4px 12px', borderRadius: '20px', border: `1px solid ${folderRoof ? accentColor : '#eee'}`, background: folderRoof ? `${accentColor}10` : '#fff', color: folderRoof ? accentColor : '#aaa', cursor: 'pointer', fontFamily: 'Montserrat,sans-serif', fontWeight: folderRoof ? 700 : 400 }}>
          {folderRoof ? (gTranslation?.recorte_casinha || '🏠 Recorte Casinha ATIVO') : (gTranslation?.recorte_reto || '⬜️ Recorte Reto ATIVO')}
        </button>
      )}
      ...
      <div style={{ textAlign: 'center' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', marginBottom: '12px' }}>
          ...
          <span style={{ fontSize: '10px', fontWeight: 800, color: '#aaa', textTransform: 'uppercase', letterSpacing: '1px' }}>{gTranslation?.titulo_lado_externo || 'LADO EXTERNO (FACE 1)'}</span>
          <div style={{ height: '1px', flex: 1, background: '#eee' }} />
        </div>
             ...
             <div style={{ position: 'absolute', top: '6px', left: '6px', right: '6px', bottom: '6px', background: '#fff', borderRadius: '1.5px', boxShadow: 'none', zIndex: 2, display: 'flex', flexDirection: 'column', alignItems: 'center', overflow: 'hidden' }}>
                <div style={{ position: 'absolute', top: '48%', left: '10px', right: '10px', zIndex: 3, display: 'flex', justifyContent: 'center', transform: 'translateY(-50%)' }}>
                  <div style={{ width: '92%', background: mainColor, borderRadius: '4px', padding: '12px 14px', textAlign: 'center', position: 'relative', border: `0.4px solid ${mainColor}`, boxShadow: 'none' }}>
                     <div style={{ fontFamily: `'Great Vibes', cursive`, color: '#fff', fontSize: '9px', marginBottom: '4px', textTransform: 'none' }}>{gTranslation?.quote_text || '"Brinque, converse e explore!"'}</div>
                     <div style={{ fontSize: '3.5px', color: '#fff', fontWeight: 500, lineHeight: 1.5, fontFamily: 'Montserrat, sans-serif' }}>
                        {gTranslation?.quote_desc || 'As brincadeiras são mais do que momentos de divertimento. Elas ajudam seu bebê a aprender, a desenvolver novas habilidades e a se sentir seguro e amado.'}
                     </div>
                  </div>
                </div>
            ...
            <div style={{ position: 'absolute', top: '10px', left: '10px', right: '10px', bottom: '10px', background: '#fff', borderRadius: '2px', boxShadow: 'none', zIndex: 2, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', border: `0.5px solid ${accentColor}15`, clipPath: folderRoof ? 'polygon(0% 12%, 50% 0%, 100% 12%, 100% 100%, 0% 100%)' : 'none', transition: 'clip-path 0.3s ease' }}>
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-start', paddingTop: '32px', textAlign: 'center', width: '100%', height: '100%' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0px' }}>
                      <div style={{ fontSize: '5.2px', fontWeight: 800, color: `${accentColor}cc`, textTransform: 'uppercase', letterSpacing: '0.6px', fontStyle: 'italic' }}>{gTranslation?.guia_de || 'GUIA DE'}</div>
                      <div style={{ fontSize: '9.2px', fontWeight: 800, color: '#333', textTransform: 'uppercase', letterSpacing: '0.8px', lineHeight: 1.2 }}>{gTranslation?.titulo_capa || 'INTRODUÇÃO ALIMENTAR'}</div>
                  </div>
                  <div style={{
                    marginTop: '5px',
                    padding: '2px 10px',
                    ...
                    }}>{gTranslation?.tagline || 'NUTRIÇÃO QUE TRANSFORMA'}</div>
                  </div>
                </div>
            </div>
          </Page>
      ...
      <div style={{ textAlign: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', marginBottom: '12px' }}>
          <div style={{ height: '1px', flex: 1, background: '#eee' }} />
          <span style={{ fontSize: '10px', fontWeight: 800, color: '#aaa', textTransform: 'uppercase', letterSpacing: '1px' }}>{gTranslation?.titulo_lado_interno || 'LADO INTERNO (FACE 2)'}</span>
          <div style={{ height: '1px', flex: 1, background: '#eee' }} />
        </div>
        <div style={{ display: 'flex', boxShadow: '0 20px 50px rgba(0,0,0,0.1)', borderRadius: '4px', overflow: 'hidden', background: '#fff' }}>
      ...
      <div style={{ width: '100%', maxWidth: '420px', background: '#fafafa', borderRadius: '10px', border: '1px solid #eee', overflow: 'hidden', zIndex: 10 }}>
        <button onClick={() => setPainelAberto(v => !v)} style={{ width: '100%', padding: '12px 16px', background: 'none', border: 'none', cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span style={{ fontSize: '11px', fontWeight: 800, color: '#666', fontFamily: 'Montserrat,sans-serif', textTransform: 'uppercase', letterSpacing: '0.5px' }}>{gTranslation?.personalizar_titulo || 'Personalizar Conteúdo do Guia Alimentar'}</span>
        ...
        {painelAberto && (
          <div style={{ padding: '0 16px 16px', display: 'flex', flexDirection: 'column', gap: '14px', maxHeight: '450px', overflowY: 'auto' }}>
            <div style={{ fontSize: '10px', fontWeight: 800, color: mainColor, fontFamily: 'Montserrat,sans-serif', textTransform: 'uppercase', borderBottom: '1px solid #eee', paddingBottom: '4px' }}>{gTranslation?.tabela_horarios || 'Tabela de Horários'}</div>
            ...
            <div style={{ fontSize: '10px', fontWeight: 800, color: mainColor, fontFamily: 'Montserrat,sans-serif', textTransform: 'uppercase', borderBottom: '1px solid #eee', paddingBottom: '4px', marginTop: '10px' }}>{gTranslation?.tabela_introducao || 'Tabela de Introdução Clássica'}</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {introducao.map((item, idx) => (
                <div key={idx} style={{ padding: '8px', border: '1px solid #eee', borderRadius: '8px', background: '#fff', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  <div style={{ fontSize: '9px', fontWeight: 800, color: '#666' }}>{gTranslation?.fase_label || 'Fase: '}{item.idade}</div>
                  
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '6px' }}>
                    <FieldRow label={gTranslation?.textura_recomendada || 'Textura Recomendada'}>
                      <input
                        value={item.text}
                    ...
                    <FieldRow label={gTranslation?.quantidade_recomendada || 'Quantidade Recomendada'}>
                      <textarea
                        value={item.qty}
