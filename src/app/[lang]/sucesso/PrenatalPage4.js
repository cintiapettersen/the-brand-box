...
export default function PrenatalPage4({ accentColor, palette = [], comBorda, patternSrc, patternScale, borderColor }) {
  ...
  const labExams = ["HB/HT", "TSH/T4L", "GLICEMIA", "TOTG", "TGO/TGP", "UREIA/CREAT", "EAS", "UROCULTURA", "HIV", "VDRL"];
  const otherExams1 = ["HTLV", "HBsAg", "HEP C"];
  const otherExams2 = ["CMV", "RUBÉOLA", "TOXOPLASMOSE"];
  const otherExams3 = ["TIPAGEM SANGUÍNEA", "ELETROFORESE HB", "COOMBS INDIRETO", "CULTURA STREPTOCOCOS", "VITAMINA D", "PESQUISA PLASMODIUM"];
      ...
      <div style={{ position: 'relative', zIndex: 2, background: '#fff', padding: '3px', height: '100%', display: 'flex', flexDirection: 'column', gap: '2px', borderRadius: '1.5px', boxShadow: '0 1px 5px rgba(0,0,0,0.05)' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', border: '0.2px solid #ccc' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1.8fr 1fr 1.5fr 1fr 1.5fr' }}>
            <TableHeader color={secondaryColor}>EXAME</TableHeader>
            <TableHeader color={secondaryColor}>DATA</TableHeader>
            <TableHeader color={secondaryColor}>RESULTADO</TableHeader>
            <TableHeader color={secondaryColor}>DATA</TableHeader>
            <TableHeader color={secondaryColor}>RESULTADO</TableHeader>
          </div>
          {labExams.map((ex, i) => (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.5fr', gap: '4px' }}>
          <div style={{ display: 'flex', flexDirection: 'column', border: '0.2px solid #ccc' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 0.8fr 1.2fr', background: '#E67E7E', color: '#fff' }}>
               <TableHeader color="#E67E7E">EXAME</TableHeader>
               <TableHeader color="#E67E7E">DATA</TableHeader>
               <TableHeader color="#E67E7E">RESULTADO</TableHeader>
            </div>
        ...
        <div style={{ display: 'flex', border: '0.2px solid #ccc' }}>
          <div style={{ flex: '1.5', display: 'flex', flexDirection: 'column' }}>
             <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', background: mainColor, color: '#fff' }}>
                <TableHeader color={mainColor}>EXAME</TableHeader>
                <TableHeader color={mainColor}>DATA</TableHeader>
             </div>
          ...
          <div style={{ flex: '1', display: 'flex', flexDirection: 'column', borderLeft: '0.2px solid #ccc' }}>
             ...
             <div style={{ flex: 1, padding: '2px' }}>
                <div style={{ fontSize: '3px', fontWeight: 700 }}>TIPAGEM SANGUÍNEA</div>
                <div style={{ fontSize: '3.5px', marginTop: '1.5px' }}>M: _________ P: __________</div>
