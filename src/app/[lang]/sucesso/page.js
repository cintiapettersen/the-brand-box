...
const COLOR_PALETTE_AFETIVA = [
  [255,198,37,'Sol de Verão'],[255,223,186,'Pêssego Suave'],[244, 201, 183, 'Pêssego Claro'],[255,182,193,'Rosa Algodão'],
  ...
  [210,140,100,'Adobe Rosado'],[188,143,143,'Rosewood Suave'],[169,147,121,'Areia Quente'],[211,176,165,'Rosa Queimado'],[205,170,125,'Linho Dourado'],
  ...
  [143,188,143,'Salvia'],[176,224,230,'Névoa Matinal'],[179,212,224,'Azul Gelo'],[135,206,250,'Céu Aberto'],
  [100,180,230,'Azul Serenidade'],[70,130,180,'Azul Aço'],[105,127,139,'Azul Mineral'],[25,90,180,'Índigo Profundo'],
  ...
  [199,199,199,'Cinza Prateado'],[180,180,180,'Cinza Névoa'],[120,120,120,'Granito'],[60,60,60,'Carvão'],
...
function GuiaStep({ brand, accentColor, paletteColors, marca, tagline, estampaPatterns, estampaSelectedIdx, editData }) {
  ...
  const safePaletteColors = Array.isArray(paletteColors) && paletteColors.length > 0 ? paletteColors : [accentColor || '#C3CEDB'];
  const safeEstampaPatterns = Array.isArray(estampaPatterns) ? estampaPatterns : [];
  const currentIdx = estampaSelectedIdx || 0;
  const patternSrc = safeEstampaPatterns[currentIdx]
    ? (safeEstampaPatterns[currentIdx].url || (safeEstampaPatterns[currentIdx].base64 ? `data:${safeEstampaPatterns[currentIdx].mimeType || 'image/png'};base64,${safeEstampaPatterns[currentIdx].base64}` : null))
    : null;
  const estiloNome = brand?.resultadoFinal?.estiloNome || brand?.estilo_nome || '';
  const mensagem = brand?.resultadoFinal?.mensagem || '';
  const fontFamily = safeEditData.fontFamily || 'Playfair Display';
  const fontWeight = safeEditData.fontWeight || 700;
  const isScript = safeEditData.fontStyle === 'script';
  const toneWords = deriveTone(estiloNome, dictionary);
  ...
  const openPrint = () => {
    ...
    let tomDeVoz = null;
    try {
      manifesto = typeof window !== 'undefined' ? localStorage.getItem(`brandbox_manifesto_${marca}`) || null : null;
    } catch {}
    try {
      const tomDeVozRaw = typeof window !== 'undefined' ? localStorage.getItem(`brandbox_tomdevoz_${marca}`) : null;
      tomDeVoz = tomDeVozRaw ? JSON.parse(tomDeVozRaw) : null;
    } catch (e) {
      console.warn('Error parsing tomDeVoz JSON:', e);
    }
    const html = buildGuiaHTML({ marca, tagline, accentColor, paletteColors: safePaletteColors, fontFamily, fontWeight, patternSrc, estiloNome, mensagem, isScript, manifesto, tomDeVoz });
...
function CartaoRetornoPreview({ accentColor, patternSrc, cartaoContacts, crmLine, editData, logoColor, comBorda, setComBorda, clinicaNome, setClinicaNome, logoLayout, paletteColors, borderColor, setBorderColor, patternScale, setPatternScale , patternOffset, setPatternOffset}) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', alignItems: 'center', width: '100%' }}>
      <BordaToggle comBorda={comBorda} setComBorda={setComBorda} accentColor={accentColor} paletteColors={paletteColors} borderColor={borderColor} setBorderColor={setBorderColor} patternScale={patternScale} setPatternScale={setPatternScale} patternOffset={typeof patternOffset !== 'undefined' ? patternOffset : 0} setPatternOffset={typeof setPatternOffset !== 'undefined' ? setPatternOffset : undefined} />
...
function OrientacoesRNPreview({ accentColor, patternSrc, editData, logoColor, logoLayout, cartaoContacts, crmLine, clinicaNome, comBorda, setComBorda, paletteColors, borderColor, setBorderColor, patternScale, setPatternScale, patternOffset = 0, setPatternOffset, rnFields = {}, setRnFields = {} }) {
      ...
      <div style={{ textAlign: 'center' }}>
        ...
        <div style={{ display: 'flex', boxShadow: '0 20px 50px rgba(0,0,0,0.1)', borderRadius: '4px', overflow: 'hidden', background: '#fff' }}>
          ...
          <Page num={5} isSmall>
            ...
            <div style={{ position: 'absolute', top: '6px', left: '6px', right: '6px', bottom: '6px', background: '#fff', borderRadius: '1.5px', boxShadow: 'none', zIndex: 2, display: 'flex', flexDirection: 'column', alignItems: 'center', overflow: 'hidden', justifyContent: 'center' }}>
          ...
          <Page num={6}>
            <div style={{ position: 'absolute', inset: 0, overflow: 'hidden', background: !comBorda ? (borderColor || paletteColors[0] || accentColor) : 'transparent' }}>
             ...
             <div style={{ position: 'absolute', top: '6px', left: '6px', right: '6px', bottom: '6px', background: '#fff', borderRadius: '1.5px', boxShadow: 'none', zIndex: 2, display: 'flex', flexDirection: 'column', alignItems: 'center', overflow: 'hidden' }}>
                <div style={{ position: 'absolute', top: '48%', left: '10px', right: '10px', zIndex: 3, display: 'flex', justifyContent: 'center', transform: 'translateY(-50%)' }}>
                  <div style={{ width: '92%', background: mainColor, borderRadius: '4px', padding: '12px 14px', textAlign: 'center', position: 'relative', border: `0.4px solid ${mainColor}`, boxShadow: 'none' }}>
...
function PapelariaStep({ brand, accentColor, paletteColors, estampaPatterns, estampaSelectedIdx, cartaoContacts, setCartaoContacts, plano, isSaude, crmData, setCrmData, marca, editData, logoColor, logoLayout, setLayout, clinicaNome, setClinicaNome, onNavSync, navIdx, setNavIdx, customLogoSrc, getCustomLogoScale, setCustomLogoScale, getCustomLogoScaleMax, customLogoScaleMap, submarcaColor, submarcaTextColor, iconPath, avulsoParam }) {
  ...
  const handleUpsellCheckout = async () => {
      ...
      try {
        ...
        if (typeof window !== 'undefined' && (new URLSearchParams(window.location.search).get('demo') === '1' || new URLSearchParams(window.location.search).get('demo') === 'BUILDWEEK100')) {
  ...
  const exportHTMLAsPDF = async (html, item, mode = 'download', wMm = null, hMm = null) => {
    ...
    try {
      ...
      for (let i = 0; i < targetElements.length; i++) {
        // Temporariamente isola o elemento alvo para evitar glitches de altura de documento no html2canvas
        for (let j = 0; j < targetElements.length; j++) {
          targetElements[j].style.display = (j === i) ? '' : 'none';
        }
        // Força o elemento a casar exatamente com as dimensões da página
        ...
        // Força o iframe a casar exatamente com uma página
...
@page { size:216mm 303mm;margin: 0 !important; } @media print { html, body { margin: 0 !important; padding: 0 !important; } .page{page-break-after:always;} }
</style></head><body>
...
@page {  size: ${210 + BLEED*2 }mm ${297 + BLEED*2}mm; margin:0; } @media print { html, body { margin: 0 !important; padding: 0 !important; } }
</style></head><body>
<div class="page" style="position:relative;">
...
@page {  size: 485.775mm 385.233mm; margin: 0 !important;  } @media print { html, body { margin: 0 !important; padding: 0 !important; } }
</style></head><body>
<div class="page">
    ${genBgP()}
...
@page {  size: ${totalW }mm ${totalH}mm; margin: 0; } @media print { html, body { margin: 0 !important; padding: 0 !important; } }
...
@page {  size: ${totalW }mm ${totalH}mm; margin: 0; } @media print { html, body { margin: 0 !important; padding: 0 !important; } .card { page-break-after: always; } }
...
.cm-bl { bottom: ${BLEED}mm; left: ${BLEED}mm; border-bottom: 0.3px solid rgba(0,0,0,0.4); border-left: 0.3px solid rgba(0,0,0,0.4); }
.cm-br { bottom: ${BLEED}mm; right: ${BLEED}mm; border-bottom: 0.3px solid rgba(0,0,0,0.4); border-right: 0.3px solid rgba(0,0,0,0.4); }
@page {  size: 56mm 96mm; margin: 0 !important;  } @media print { html, body { margin: 0 !important; padding: 0 !important; } .card { page-break-after: always; } }
...
@page {  size: ${_cW } ${_cH}; margin: 0; } @media print { html, body { margin: 0 !important; padding: 0 !important; } .card { page-break-after: always; } }
</style>
</head>
<body>
${frenteHtml}
...
@page {  size: 220mm 307mm; margin: 0 !important;  } @media print { html, body { margin: 0 !important; padding: 0 !important; } }
...
<style>* { box-sizing:border-box; margin:0; padding:0; print-color-adjust:exact !important; -webkit-print-color-adjust:exact !important; }
@page {  size: ${_pw }mm ${_ph}mm; margin:0; } html, body { margin: 0 !important; padding: 0 !important; } @media print { }
...
@page {  size: 303mm 216mm; margin: 0 !important;  } @media print { html, body { margin: 0 !important; padding: 0 !important; } }
</style></head><body>
<div class="page">
    ${genBg(10)}
    <div style="position:absolute;top:${BLEED}mm;left:0;width:${BLEED-0.5}mm;height:0.2mm;background:#000;z-index:100;"></div>
...
@page {  size: ${W + BLEED*2 }mm ${H + BLEED*2}mm; margin: 0; } @media print { html, body { margin: 0 !important; padding: 0 !important; } }
</style></head><body>
...
<div style="width:${totalW}mm;height:${totalH}mm;position:relative;overflow:hidden;">
  ...
  <div style="position:absolute;bottom:${BLEED}mm;right:${BLEED}mm;width:8mm;height:0.3mm;background:#fff;opacity:0.5;"></div><div style="position:absolute;bottom:${BLEED}mm;right:${BLEED}mm;width:0.3mm;height:8mm;background:#fff;opacity:0.5;"></div>
      ...
      // Logo — usa genPDFLogoHtml com a mesma proporção do Preview (caixa branca de ~75% e escala cheia)
...
.cm-bl { bottom:${BLEED}mm; left:${BLEED}mm; border-bottom:0.2mm solid; border-left:0.2mm solid; }
.cm-br { bottom:${BLEED}mm; right:${BLEED}mm; border-bottom:0.2mm solid; border-right:0.2mm solid; }
.field { border-bottom: 0.2mm solid #ddd; padding: 1mm 0; font-size: 6pt; color: #333; margin-top: 2mm; display:flex; gap: 4mm; align-items:flex-end; }
.label { font-weight: 700; color: #1a1a1a; text-transform: uppercase; font-size: 5pt; flex-shrink: 0; margin-bottom: 0.5mm; }
...
@page {  size: ${totalW }mm ${totalH}mm; margin:0; } @media print { html, body { margin: 0 !important; padding: 0 !important; } .page { page-break-after:always; border:none; } }
</style></head><body>
...
@page {  size: ${totalW }mm ${totalH}mm; margin:0; } @media print { html, body { margin: 0 !important; padding: 0 !important; } .page { page-break-after:always; } }
</style></head><body>
...
@page {  size: ${wMm }mm ${hMm}mm; margin:0; } @media print { html, body { margin: 0 !important; padding: 0 !important; } }
</style></head><body>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', paddingTop: '4px', paddingBottom: '8px', width: '100%' }}>
        <UniversalPreviewScaler targetWidth={getPreviewTargetWidth(currentItem)}>
          {currentItem.includes('Cartão de Visita')
            ? <CartaoDeVisitaPreview accentColor={accentColor} patternSrc={patternSrc} cartaoContacts={cartaoContacts} crmLine={crmLine} editData={{ ...itemEditData, tagline: localSlogan }} logoColor={logoColor} comBorda={comBorda} setComBorda={setComBorda} clinicaNome={clinicaNome} setClinicaNome={setClinicaNome} logoLayout={logoLayout} paletteColors={paletteColors} borderColor={borderColor} setBorderColor={setBorderColor} patternScale={patternScale} setPatternScale={setPatternScale} patternOffset={typeof patternOffset !== 'undefined' ? patternOffset : 0} setPatternOffset={typeof setPatternOffset !== 'undefined' ? setPatternOffset : undefined} retrato={cartaoRetrato} setRetrato={setCartaoRetrato} />
            : currentItem.includes('Envelope Ofício')
              ? <EnvelopeOficioPreview accentColor={accentColor} patternSrc={patternSrc} logoColor={logoColor} logoLayout={logoLayout} comBorda={comBorda} setComBorda={setComBorda} paletteColors={paletteColors} borderColor={borderColor} setBorderColor={setBorderColor} patternScale={patternScale} setPatternScale={setPatternScale} patternOffset={typeof patternOffset !== 'undefined' ? patternOffset : 0} setPatternOffset={typeof setPatternOffset !== 'undefined' ? setPatternOffset : undefined} cartaoContacts={cartaoContacts} crmLine={crmLine} localSlogan={localSlogan} brand={brand} editData={itemEditData} clinicaNome={clinicaNome} storyTemplateIdx={storyTemplateIdx} setStoryTemplateIdx={setStoryTemplateIdx} storyFormatIdx={storyFormatIdx} setStoryFormatIdx={setStoryFormatIdx} />
            : currentItem.includes('Envelope Saco')
              ? <EnvelopeSacoPreview accentColor={accentColor} patternSrc={patternSrc} logoColor={logoColor} logoLayout={logoLayout} comBorda={comBorda} setComBorda={setComBorda} paletteColors={paletteColors} borderColor={borderColor} setBorderColor={setBorderColor} patternScale={patternScale} setPatternScale={setPatternScale} patternOffset={typeof patternOffset !== 'undefined' ? patternOffset : 0} setPatternOffset={typeof setPatternOffset !== 'undefined' ? setPatternOffset : undefined} cartaoContacts={cartaoContacts} crmLine={crmLine} localSlogan={localSlogan} brand={brand} editData={itemEditData} clinicaNome={clinicaNome} storyTemplateIdx={storyTemplateIdx} setStoryTemplateIdx={setStoryTemplateIdx} storyFormatIdx={storyFormatIdx} setStoryFormatIdx={setStoryFormatIdx} />
            : currentItem === 'Recibo'
              ? <ReciboPreview accentColor={accentColor} patternSrc={patternSrc} cartaoContacts={cartaoContacts} crmLine={crmLine} editData={{ ...itemEditData, tagline: localSlogan }} logoColor={logoColor} comBorda={comBorda} setComBorda={setComBorda} clinicaNome={clinicaNome} setClinicaNome={setClinicaNome} logoLayout={logoLayout} paletteColors={paletteColors} borderColor={borderColor} setBorderColor={setBorderColor} patternScale={patternScale} setPatternScale={setPatternScale} patternOffset={typeof patternOffset !== 'undefined' ? patternOffset : 0} setPatternOffset={typeof setPatternOffset !== 'undefined' ? setPatternOffset : undefined} marca={marca} />
            : currentItem.includes('Cartão de Retorno')
              ? <CartaoRetornoPreview accentColor={accentColor} patternSrc={patternSrc} cartaoContacts={cartaoContacts} crmLine={crmLine} editData={{ ...itemEditData, tagline: localSlogan }} logoColor={logoColor} comBorda={comBorda} setComBorda={setComBorda} clinicaNome={clinicaNome} setClinicaNome={setClinicaNome} logoLayout={logoLayout} paletteColors={paletteColors} borderColor={borderColor} setBorderColor={setBorderColor} patternScale={patternScale} setPatternScale={setPatternScale} patternOffset={typeof patternOffset !== 'undefined' ? patternOffset : 0} setPatternOffset={typeof setPatternOffset !== 'undefined' ? setPatternOffset : undefined} />
            : currentItem.includes('Agradecimento')
              ? <CartaoAgradecimentoPreview accentColor={accentColor} patternSrc={patternSrc} editData={{ ...itemEditData, tagline: localSlogan }} logoColor={logoColor} logoLayout={logoLayout} cartaoContacts={cartaoContacts} crmLine={crmLine} clinicaNome={clinicaNome} comBorda={comBorda} setComBorda={setComBorda} paletteColors={paletteColors} borderColor={borderColor} setBorderColor={setBorderColor} patternScale={patternScale} setPatternScale={setPatternScale} patternOffset={typeof patternOffset !== 'undefined' ? patternOffset : 0} setPatternOffset={typeof setPatternOffset !== 'undefined' ? setPatternOffset : undefined} sizeIdx={agradecimentoSizeIdx} setSizeIdx={setAgradecimentoSizeIdx} msgIdx={agradecimentoMsgIdx} setMsgIdx={setAgradecimentoMsgIdx} />
            : currentItem === 'Ficha de Cadastro'
              ? <FichaCadastroPreview accentColor={accentColor} patternSrc={patternSrc} editData={{ ...itemEditData, tagline: localSlogan }} logoColor={logoColor} logoLayout={logoLayout} cartaoContacts={cartaoContacts} crmLine={crmLine} clinicaNome={clinicaNome} comBorda={comBorda} setComBorda={setComBorda} paletteColors={paletteColors} borderColor={borderColor} setBorderColor={setBorderColor} patternScale={patternScale} setPatternScale={setPatternScale} patternOffset={typeof patternOffset !== 'undefined' ? patternOffset : 0} setPatternOffset={typeof setPatternOffset !== 'undefined' ? setPatternOffset : undefined} fichaAdulto={fichaAdulto} setFichaAdulto={setFichaAdulto} />
            ...
            : currentItem === 'Gráfico de Crescimento'
              ? <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', alignItems: 'center' }}>
                  <BordaToggle comBorda={comBorda} setComBorda={setComBorda} accentColor={accentColor} paletteColors={paletteColors} borderColor={borderColor} setBorderColor={setBorderColor} patternScale={patternScale} setPatternScale={setPatternScale} patternOffset={typeof patternOffset !== 'undefined' ? patternOffset : 0} setPatternOffset={typeof setPatternOffset !== 'undefined' ? setPatternOffset : undefined} />
                  <GraficoCrescimentoPreview accentColor={accentColor} paletteColors={paletteColors} editData={itemEditData} logoColor={logoColor} logoLayout={logoLayout} cartaoContacts={cartaoContacts} crmLine={crmLine} clinicaNome={clinicaNome} comBorda={comBorda} setComBorda={setComBorda} borderColor={borderColor} setBorderColor={setBorderColor} patternSrc={patternSrc} patternScale={patternScale} setPatternScale={setPatternScale} patternOffset={typeof patternOffset !== 'undefined' ? patternOffset : 0} setPatternOffset={typeof setPatternOffset !== 'undefined' ? setPatternOffset : undefined} />
                </div>
            : currentItem === 'Diário do Xixi'
              ? <DiarioXixiPreview accentColor={accentColor} patternSrc={patternSrc} editData={{ ...itemEditData, tagline: localSlogan }} logoColor={logoColor} logoLayout={logoLayout} cartaoContacts={cartaoContacts} crmLine={crmLine} clinicaNome={clinicaNome} comBorda={comBorda} setComBorda={setComBorda} paletteColors={paletteColors} borderColor={borderColor} setBorderColor={setBorderColor} patternScale={patternScale} setPatternScale={setPatternScale} patternOffset={typeof patternOffset !== 'undefined' ? patternOffset : 0} setPatternOffset={typeof setPatternOffset !== 'undefined' ? setPatternOffset : undefined} />
            : currentItem === 'Receita de Alta'
              ? <ReceitaAltaPreview accentColor={accentColor} patternSrc={patternSrc} editData={{ ...itemEditData, tagline: localSlogan }} logoColor={logoColor} logoLayout={logoLayout} cartaoContacts={cartaoContacts} crmLine={crmLine} clinicaNome={clinicaNome} comBorda={comBorda} setComBorda={setComBorda} paletteColors={paletteColors} borderColor={borderColor} setBorderColor={setBorderColor} patternScale={patternScale} setPatternScale={setPatternScale} patternOffset={typeof patternOffset !== 'undefined' ? patternOffset : 0} setPatternOffset={typeof setPatternOffset !== 'undefined' ? setPatternOffset : undefined} receitaFields={receitaFields} setReceitaFields={setReceitaFields} />
            : currentItem === 'Caneca' || currentItem === 'Arte para Caneca'
              ? <CanecaPreview accentColor={accentColor} patternSrc={patternSrc} editData={{ ...itemEditData, tagline: localSlogan }} logoColor={logoColor} logoLayout={logoLayout} cartaoContacts={cartaoContacts} crmLine={crmLine} clinicaNome={clinicaNome} comBorda={comBorda} setComBorda={setComBorda} paletteColors={paletteColors} borderColor={borderColor} setBorderColor={setBorderColor} patternScale={patternScale} setPatternScale={setPatternScale} patternOffset={typeof patternOffset !== 'undefined' ? patternOffset : 0} setPatternOffset={typeof setPatternOffset !== 'undefined' ? setPatternOffset : undefined} submarcaColor={submarcaColor} submarcaTextColor={submarcaTextColor} iconPath={iconPath} />
            : currentItem === 'T-Shirt'
              ? <CamisetaPreview accentColor={accentColor} patternSrc={patternSrc} editData={{ ...itemEditData, tagline: localSlogan }} logoColor={logoColor} logoLayout={logoLayout} cartaoContacts={cartaoContacts} crmLine={crmLine} clinicaNome={clinicaNome} comBorda={comBorda} setComBorda={setComBorda} paletteColors={paletteColors} borderColor={borderColor} setBorderColor={setBorderColor} patternScale={patternScale} setPatternScale={setPatternScale} patternOffset={typeof patternOffset !== 'undefined' ? patternOffset : 0} setPatternOffset={typeof setPatternOffset !== 'undefined' ? setPatternOffset : undefined} submarcaColor={submarcaColor} submarcaTextColor={submarcaTextColor} iconPath={iconPath} />
            : currentItem === 'Papel de Presente'
              ? <PapelPresentePreview accentColor={accentColor} paletteColors={paletteColors} comBorda={comBorda} setComBorda={setComBorda} patternSrc={patternSrc} patternScale={patternScale} setPatternScale={setPatternScale} patternOffset={typeof patternOffset !== 'undefined' ? patternOffset : 0} setPatternOffset={typeof setPatternOffset !== 'undefined' ? setPatternOffset : undefined} borderColor={borderColor} setBorderColor={setBorderColor} sizeIdx={papelPresenteSizeIdx} setSizeIdx={setPapelPresenteSizeIdx} />
            : currentItem === 'Tag para Sacola'
              ? <TagSacolaPreview item={currentItem} accentColor={accentColor} patternSrc={patternSrc} editData={{ ...itemEditData, tagline: localSlogan }} logoColor={logoColor} logoLayout={logoLayout} cartaoContacts={cartaoContacts} crmLine={crmLine} clinicaNome={clinicaNome} comBorda={comBorda} setComBorda={setComBorda} paletteColors={paletteColors} borderColor={borderColor} setBorderColor={setBorderColor} patternScale={patternScale} setPatternScale={setPatternScale} patternOffset={typeof patternOffset !== 'undefined' ? patternOffset : 0} setPatternOffset={typeof setPatternOffset !== 'undefined' ? setPatternOffset : undefined} sizeIdx={tagSacolaSizeIdx} setSizeIdx={setTagSacolaSizeIdx} />
          : currentItem === 'Caixa Gaveta (L 13,5 x P 18,5 cm)'
            ...
            : currentItem === 'Etiqueta para Correios'
              ? <EtiquetaCorreiosPreview accentColor={accentColor} patternSrc={patternSrc} editData={{ ...itemEditData, tagline: localSlogan }} logoColor={logoColor} logoLayout={logoLayout} cartaoContacts={cartaoContacts} crmLine={crmLine} clinicaNome={clinicaNome} comBorda={comBorda} setComBorda={setComBorda} paletteColors={paletteColors} borderColor={borderColor} setBorderColor={setBorderColor} patternScale={patternScale} setPatternScale={setPatternScale} patternOffset={typeof patternOffset !== 'undefined' ? patternOffset : 0} setPatternOffset={typeof setPatternOffset !== 'undefined' ? setPatternOffset : undefined} sizeIdx={etiquetaSizeIdx} setSizeIdx={setEtiquetaSizeIdx} fraseIdx={etiquetaFraseIdx} setFraseIdx={setEtiquetaFraseIdx} />
            ...
            : currentItem.includes('Pré-Natal')
              ? <FolderA5Preview brand={brand} editData={itemEditData} logoColor={logoColor} logoLayout={logoLayout} comBorda={comBorda} setComBorda={setComBorda} patternSrc={patternSrc} patternScale={patternScale} setPatternScale={setPatternScale} patternOffset={typeof patternOffset !== 'undefined' ? patternOffset : 0} setPatternOffset={typeof setPatternOffset !== 'undefined' ? setPatternOffset : undefined} accentColor={accentColor} borderColor={borderColor} setBorderColor={setBorderColor} paletteColors={paletteColors} title={currentItem} cartaoContacts={cartaoContacts} crmLine={crmLine} folderRoof={folderRoof} />
            : (currentItem === 'Guia Alimentar' || currentItem === 'Dietary Guide' || currentItem.includes('Alimentar'))
            ...
            : currentItem.includes('Caderno')
              ? <CadernoPreview editData={{ ...itemEditData, tagline: localSlogan }} accentColor={accentColor} solidColor={paletteColors[0]} logoColor={logoColor} logoLayout={logoLayout} comBorda={comBorda} setComBorda={setComBorda} patternSrc={patternSrc} paletteColors={paletteColors} borderColor={borderColor} setBorderColor={setBorderColor} patternScale={patternScale} setPatternScale={setPatternScale} patternOffset={typeof patternOffset !== 'undefined' ? patternOffset : 0} setPatternOffset={typeof setPatternOffset !== 'undefined' ? setPatternOffset : undefined} paperSize={paperSize} setPaperSize={setPaperSize} cartaoContacts={cartaoContacts} clinicaNome={clinicaNome} />
...
class PostPaymentErrorBoundary extends React.Component {
  constructor(props) {
    super(props);
  ...
  componentDidCatch(error, errorInfo) {
    console.error('PostPaymentErrorBoundary caught error:', error, errorInfo);
    if (typeof window !== 'undefined' && this.props.brandId) {
      try {
        localStorage.removeItem(`brandbox_step_${this.props.brandId}`);
        localStorage.removeItem('brandbox_step');
      } catch (e) {}
    }
  }
  handleReset = () => {
    if (this.props.onResetStep) {
  ...
  render() {
    if (this.state.hasError) {
      return (
        <div style={{ background: '#fff0f0', border: '1.5px solid #ffcccc', borderRadius: '16px', padding: '24px', textAlign: 'center', margin: '20px 0' }}>
          <div style={{ fontSize: '2rem', marginBottom: '8px' }}>⚠️</div>
          <h3 style={{ fontSize: '1rem', color: '#c53030', marginBottom: '8px', fontFamily: 'Montserrat, sans-serif' }}>Ops! Ocorreu um problema nesta visualização.</h3>
          <p style={{ fontSize: '0.82rem', color: '#666', marginBottom: '16px', lineHeight: 1.5, fontFamily: 'Montserrat, sans-serif' }}>
            Não se preocupe, seus dados estão salvos. Você pode voltar para a etapa inicial da sua marca sem perder o progresso.
          </p>
          <button
            onClick={this.handleReset}
            style={{ padding: '10px 20px', background: '#363532', color: '#fff', border: 'none', borderRadius: '20px', fontWeight: 700, fontSize: '0.85rem', cursor: 'pointer', fontFamily: 'Montserrat, sans-serif' }}
          >
...
function EntregaContent({ brand, plano, setBrand }) {
  ...
  const setPatternOffset = (v) => {
    ...
    try {
      if (brand?.id) localStorage.setItem(`brandbox_pattern_offset_${brand.id}`, String(v));
    } catch {}
  };
  ...
  useEffect(() => {
    ...
    try {
      const s = localStorage.getItem(`brandbox_step_${brand.id}`);
      if (s && ALL_STEPS.includes(s)) {
        setStepState(s);
      } else if (s) {
        try { localStorage.removeItem(`brandbox_step_${brand.id}`); } catch {}
      }
