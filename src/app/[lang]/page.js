...
import { BRAND_ELEMENTS_COPY, invalidateBrandElementsForNewPattern, selectedBrandElement as resolveSelectedBrandElement } from '../../lib/brandElements';
...
export default function Home() {
  ...
  const brandElementCopy = lang === 'en' ? BRAND_ELEMENTS_COPY.en : BRAND_ELEMENTS_COPY.pt;
  ...
  const [brandElements, setBrandElements] = useState({ status: 'idle', analysis: null, elements: [], selectedId: null, context: null, generationUsed: false, sourcePatternIndex: null, error: null });
  ...
  const brandElementRequestRef = useRef(null);
  ...
  const activeBrandElement = brandElements.sourcePatternIndex === selectedPattern ? resolveSelectedBrandElement(brandElements, { journeyId: resultadoFinal?.creativeDirectorJourneyId || null, styleId: String(resultadoFinal?.estiloId || ''), patternHash: brandElements.context?.patternHash, paletteId: selectedPaletteDetails?.id || null, paletteHex: selectedPaletteDetails?.hex || [] }) : null;
  const activeBrandElementSrc = activeBrandElement ? `data:${activeBrandElement.mimeType};base64,${activeBrandElement.base64}` : null;
  ...
  const restoreProgress = async (parsed) => {
    ...
    if (parsed.brandElements) setBrandElements(parsed.brandElements);
  ...
  useEffect(() => {
    ...
    const dataToSave = {
      ...
      generatedPatterns, selectedPattern, brandElements, papelariaSelecionada,
  ...
  const invalidateBrandElementsForPatternChange = () => {
    setBrandElements(prev => invalidateBrandElementsForNewPattern(prev, lang));
  };

  const selectApprovedPattern = (index) => {
    if (index !== selectedPattern) invalidateBrandElementsForPatternChange();
    setSelectedPattern(index);
  };

  const generateBrandElements = async () => {
    const approvedPattern = generatedPatterns[selectedPattern];
    if (!approvedPattern?.base64 || brandElements.generationUsed || brandElementRequestRef.current) return;
    const selectedPalette = findSelectedPalette(paletas, selectedPaleta, { styleId: resultadoFinal?.estiloId, styleName: resultadoFinal?.estiloNome, journeyId: resultadoFinal?.creativeDirectorJourneyId });
    if (!selectedPalette) return;
    const requestKey = `${resultadoFinal?.creativeDirectorJourneyId}:${selectedPattern}:${selectedPalette.id}`;
    brandElementRequestRef.current = requestKey;
    setBrandElements(prev => ({ ...prev, status: 'analyzing', error: null, selectedId: null }));
    try {
      const common = { journeyId: resultadoFinal?.creativeDirectorJourneyId, styleId: resultadoFinal?.estiloId, styleName: resultadoFinal?.estiloNome, paletteId: selectedPalette.id, paletteHex: selectedPalette.hex, accentColor: editData.corAtiva, language: lang, approvedPattern: { base64: approvedPattern.base64, mimeType: approvedPattern.mimeType } };
      const analysisResponse = await fetch('/api/creative-director/brand-elements/analyze-pattern', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...common, briefing: formData }) });
      if (!analysisResponse.ok) throw new Error('analysis_failed');
      const analysis = await analysisResponse.json();
      if (brandElementRequestRef.current !== requestKey) return;
      setBrandElements(prev => ({ ...prev, status: 'generating', analysis: analysis.motifs, context: analysis.context }));
      const generationResponse = await fetch('/api/creative-director/brand-elements/generate', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ approvedPattern: common.approvedPattern, motifs: analysis.motifs, context: analysis.context }) });
      if (!generationResponse.ok) throw new Error('generation_failed');
      const generated = await generationResponse.json();
      if (brandElementRequestRef.current !== requestKey || generated.elements?.length !== 3) return;
      setBrandElements({ status: 'ready', analysis: analysis.motifs, elements: generated.elements, selectedId: null, context: generated.context, generationUsed: true, sourcePatternIndex: selectedPattern, error: null });
    } catch {
      if (brandElementRequestRef.current === requestKey) setBrandElements(prev => ({ ...prev, status: 'error', error: lang === 'en' ? 'We could not create the graphic elements right now. Please try again; your generation was not used.' : 'Não foi possível criar os elementos gráficos agora. Tente novamente; sua geração não foi utilizada.' }));
    } finally { if (brandElementRequestRef.current === requestKey) brandElementRequestRef.current = null; }
  };
  ...
  const generatePatterns = async () => {
    ...
    invalidateBrandElementsForPatternChange();
  ...
  const selectPaletteForColor = (paletteId) => {
    ...
    if (brandElements.generationUsed && brandElements.context?.paletteId !== String(paletteId)) setBrandElements(prev => ({ ...invalidateBrandElementsForNewPattern(prev, lang), error: lang === 'en' ? 'These elements belonged to the previous palette. The pre-purchase generation has already been used.' : 'Estes elementos pertenciam à paleta anterior. A geração pré-compra já foi utilizada.' }));
  ...
  const handleCheckoutPro = async () => {
    ...
    try {
      ...
      const brandState = {
        ...
        selectedPaleta, selectedIcon, selectedTipo, brandElements, selectedBrandElement: brandElements.selectedId,
        paletas, tipografias,
                  ...
                  <div style={{ display: 'flex', gap: '10px' }}>
                    ...
                    <button
                      onClick={() => {
                        ...
                        setBrandElements({ status: 'idle', analysis: null, elements: [], selectedId: null, context: null, generationUsed: false, sourcePatternIndex: null, error: null });
                        setSelectedPaleta(null);
                        setEditData(prev => ({ ...prev, corAtiva: null }));
                        setSelectedTipo(null);
                        setStep(1);
                  ...
                  <>
                    ...
                    <div style={{ display: 'flex', gap: '20px', width: '100%', justifyContent: 'center' }}>
                      {generatedPatterns.slice(0, 2).map((p, i) => (
                        <div
                          key={i}
                          onClick={() => selectApprovedPattern(i)}
                          style={{
                    ...
                    {selectedPattern !== null && !generatedPatterns[selectedPattern]?._devPlaceholder && (
                      <section aria-labelledby="brand-elements-title" style={{ width: '100%', maxWidth: '520px', padding: '16px 12px 22px', border: '1px solid var(--border)', borderRadius: '20px', background: '#fafafa', marginTop: '8px' }}>
                        <h3 id="brand-elements-title" style={{ fontSize: '0.92rem', textAlign: 'center', marginBottom: '6px' }}>{brandElementCopy.title}</h3>
                        <p style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', lineHeight: 1.5, textAlign: 'center', margin: '0 auto 12px', maxWidth: '430px' }}>{brandElementCopy.description}</p>
                        {['idle', 'error'].includes(brandElements.status) && !brandElements.generationUsed && <button type="button" onClick={generateBrandElements} disabled={Boolean(brandElementRequestRef.current)} className="btn-secondary" style={{ width: '100%', padding: '11px 14px' }}>{brandElementCopy.cta}</button>}
                        {['analyzing', 'generating'].includes(brandElements.status) && <div role="status" aria-live="polite" aria-busy="true" style={{ textAlign: 'center', color: 'var(--text-secondary)', fontSize: '0.78rem', padding: '12px' }}><PaletteLoadingSparkle /> {brandElements.status === 'analyzing' ? brandElementCopy.analyzing : brandElementCopy.generating}</div>}
                        {brandElements.error && <p role="status" style={{ color: brandElements.status === 'stale' ? '#9a3412' : '#b42318', fontSize: '0.76rem', lineHeight: 1.45, textAlign: 'center', margin: '10px 0' }}>{brandElements.error}</p>}
                        {brandElements.status === 'ready' && brandElements.elements?.length === 3 && <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.25 }}><div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))', gap: '8px' }}>{brandElements.elements.map(element => { const selected = brandElements.selectedId === element.id; const src = `data:${element.mimeType};base64,${element.base64}`; return <button type="button" key={element.id} aria-pressed={selected} onClick={() => setBrandElements(prev => ({ ...prev, selectedId: element.id }))} style={{ position: 'relative', minWidth: 0, border: selected ? '3px solid var(--accent-turquoise)' : '1px solid #ddd', borderRadius: '16px', background: '#fff', padding: '8px', cursor: 'pointer', boxShadow: selected ? '0 8px 18px rgba(42,137,127,.25)' : 'none' }}><div style={{ display: 'flex', gap: '4px', justifyContent: 'center' }}><img src={src} alt="" style={{ width: '42px', height: '42px', objectFit: 'contain' }} /><span aria-hidden="true" style={{ width: '42px', height: '42px', display: 'block', backgroundColor: editData.corAtiva || '#111111', WebkitMaskImage: `url(${src})`, maskImage: `url(${src})`, WebkitMaskSize: 'contain', maskSize: 'contain', WebkitMaskRepeat: 'no-repeat', maskRepeat: 'no-repeat', WebkitMaskPosition: 'center', maskPosition: 'center' }} /></div><strong style={{ display: 'block', fontSize: '0.66rem', marginTop: '5px', lineHeight: 1.2 }}>{element.name}</strong><span style={{ display: 'block', fontSize: '0.56rem', color: '#667085', lineHeight: 1.3, marginTop: '3px' }}>{element.origin}</span>{selected && <span aria-hidden="true" style={{ position: 'absolute', top: '5px', right: '5px', width: '19px', height: '19px', borderRadius: '50%', background: '#fff', display: 'grid', placeItems: 'center', boxShadow: '0 1px 4px rgba(0,0,0,.2)' }}>✓</span>}</button>})}</div><button type="button" onClick={() => setBrandElements(prev => ({ ...prev, selectedId: null }))} className="btn-secondary" style={{ width: '100%', marginTop: '10px', padding: '9px' }}>{brandElementCopy.keepIcon}</button></motion.div>}
                      </section>
                    )}
                  </>
                )}
              ...
              <div className="brand-board-container" style={{ flex: 1, overflowY: 'auto', display: 'flex', justifyContent: 'center', alignItems: 'flex-start', padding: '20px' }}>
                <div ref={brandBoardRef} className="brand-board-wrapper">
                  ...
                  <BrandBoard 
                    ...
                    iconPath={activeBrandElementSrc || getIconById(ESTILO_NOME_BY_ID[resultadoFinal?.estiloId] || resultadoFinal?.estiloNome, selectedIcon)?.path || null}
                    iconIsMask={Boolean(activeBrandElementSrc)}
                  />
                </div>
              </div>
              ...
              {(() => {
                ...
                return (
                  <div style={{ padding: '10px 20px', background: '#fff', borderTop: '1px solid #f0f0f0', display: 'flex', alignItems: 'center', gap: '10px', justifyContent: 'center' }}>
                    ...
                    {/* opção nenhum */}
                    <div
                      onClick={() => { setSelectedIcon(null); setBrandElements(prev => ({ ...prev, selectedId: null })); }}
                      title="Nenhum"
                      style={{
                    ...
                    {styleIcons.slice(0, 5).map(icon => (
                      <div
                        key={icon.id}
                        onClick={() => { setSelectedIcon(icon.id); setBrandElements(prev => ({ ...prev, selectedId: null })); }}
                        title={icon.label}
                        style={{
                          width: '38px', height: '38px', borderRadius: '50%', cursor: 'pointer',
