...
export async function POST(request) {
  ...
  try {
    ...
    const feedback = {
      ...
      comment: clean(body.feedback?.comment).slice(0, 400)
    };
    ...
    if (completed.size >= PALETTE_CONSULTATION_LIMIT && !completed.has(consultationIndex)) {
      return Response.json({ error: 'palette_consultation_limit_reached' }, { status: 429 });
    }
    ...
    let payload;
    try {
      payload = JSON.parse(outputText(await response.json()));
    } catch {
      throw Object.assign(new Error('invalid_json'), { publicCode: 'palette_consultation_invalid_json', phase: 'json', errorId: 'invalid_json' });
    }
